...
| **FR-17** | The UI shall provide **section navigation**: collapsible **Colima** and **Docker** groups in the sidebar; **Colima** views — **Runtime** (actions + start options + telemetry), **Profiles** (table from `colima list -j`), **Template** (YAML + edit in terminal); **Docker** views — **Containers**, **Images**, **Volumes**, **Networks**; a **Logs** control under Refresh opens a **command log** view. | Should | Implemented | `renderer/sidebar.js`, `index.html`, `styles.css` |
...
| **FR-20** | **Kubernetes:** when `COLIMA_UI_K8S_ENABLED=1`, the app may expose read-only **kubectl** probes (e.g. nodes); otherwise return a stub. | Could | Partial | IPC `kubernetes:getNodes`, `domain/kubernetes/kubernetes-operations.js` |

### 3.1 Explicitly out of scope (POC)

| ID | Requirement | Rationale |
|----|-------------|-----------|
| **FR-X-01** | Drive **kubectl**, **Helm**, **istioctl**, or Istio addons from the UI beyond optional stub | Covered by **`workflow.md`**; separate product surface |
...
| FR-20 | `domain/kubernetes/kubernetes-operations.js`, `main.js`, `preload.js` |
