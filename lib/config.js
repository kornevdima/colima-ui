...
const config = Object.freeze({
  ...
  kubernetes: Object.freeze({
    ...
    /**
     * `kubectl get svc -n <ns>`. Unset defaults to `istio-system`. Set to empty string
     * (`COLIMA_UI_K8S_SERVICES_NAMESPACE=`) to use `-A` (all namespaces).
     */
    servicesNamespace: (() => {
      const raw = process.env.COLIMA_UI_K8S_SERVICES_NAMESPACE;
      if (raw === undefined) return "istio-system";
  ...
  /** UI-only / cross-cutting (not all keys exposed as COLIMA_UI_* elsewhere). */
  ui: Object.freeze({
    /**
     * Linux: terminal binary for `launchInTerminal`. COLIMA_UI_TERMINAL_LINUX, else TERMINAL env at launch.
     */
    terminalLinux: (() => {
      const a = process.env.COLIMA_UI_TERMINAL_LINUX;
      if (a !== undefined && a !== "") return a;
      const b = process.env.TERMINAL;
